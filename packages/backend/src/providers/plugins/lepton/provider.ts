...
const BASE = 'https://llama3-1-70b.lepton.run/api/v1'
const KEY_ENV = 'LEPTON_API_KEY'
...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`lepton ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'lepton', displayName:'Lepton AI', description:'Lepton AI — OpenAI-compatible API', signupUrl:'',
