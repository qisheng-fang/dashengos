...
const KEY_ENV = 'DEEPINFRA_API_KEY'
...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`deepinfra ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'deepinfra', displayName:'DeepInfra', description:'DeepInfra — OpenAI-compatible API', signupUrl:'',
