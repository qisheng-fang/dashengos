...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`novita ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'novita', displayName:'Novita AI', description:'Novita AI — OpenAI-compatible API', signupUrl:'',
