...
const KEY_ENV = 'CLOUDFLARE_API_KEY'
...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`cloudflare ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'cloudflare', displayName:'Cloudflare Workers AI', description:'Cloudflare Workers AI — OpenAI-compatible API', signupUrl:'',
