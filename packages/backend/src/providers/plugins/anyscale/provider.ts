...
const BASE = 'https://api.endpoints.anyscale.com/v1'
const KEY_ENV = 'ANYSCALE_API_KEY'
...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`anyscale ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'anyscale', displayName:'Anyscale', description:'Anyscale — OpenAI-compatible API', signupUrl:'',
