...
async function chatImpl(req: ChatRequest, apiKey: string): Promise<ChatResponse> {
  ...
  if (!resp.ok) { const t = await resp.text(); throw new Error(`fireworks ${resp.status}: ${t.slice(0,200)}`) }
...
const profile: ProviderProfile = {
  name:'fireworks', displayName:'Fireworks AI', description:'Fireworks AI — OpenAI-compatible API', signupUrl:'',
